...
async fn main() {
    ...
    println!("{:#?}", std_lib::string::concat(vec! ["Sum 1-10: ".to_string(),from(sum.clone())]));
    println!("{:#?}", std_lib::string::concat(vec! ["Product 1-5: ".to_string(),from(product.clone())]));
    ...
    println!("{:#?}", std_lib::string::concat(vec! ["Sum of squares 1-5: ".to_string(),from(sum_of_squares.clone())]));
