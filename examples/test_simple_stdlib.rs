...
async fn main() {
    ...
    let len_str: String = std_lib::convert::to_string(len.clone());
    println!("{}", len_str);
    let x_val: f64 = -5.7;
    let abs_x: f64 = std_lib::math::abs(x_val.clone());
    let abs_str: String = std_lib::convert::to_string(abs_x.clone());
    println!("{}", abs_str);
    let sqrt_val: f64 = std_lib::math::sqrt(16.0);
    let sqrt_str: String = std_lib::convert::to_string(sqrt_val.clone());
    println!("{}", sqrt_str);
    let numbers: Vec<i64> = vec! [1, 2, 3, 4, 5];
    let array_length: i64 = std_lib::array::len(numbers.clone());
    let length_str: String = std_lib::convert::to_string(array_length.clone());
    println!("{}", length_str);
