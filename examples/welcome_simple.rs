...
async fn main() {
    ...
    let result: i64 = match     safe_divide(10, 2).await { Ok(v) => v, Err(e) => (|err: String| -> i64 { println!("{}", std_lib::string::concat(vec! ["Error occurred: ".string_from(), err.clone()]));     return -1 })(e) };
    ...
    println!("{}", std_lib::string::concat(vec! ["5 + 3 = ".string_from(), string_from(total.clone())]));
