...
async fn main() {
    ...
    let health_str: String = from(player_health.clone());
    let level_str: String = from(player_level.clone());
    let gold_str: String = from(player_gold.clone());
    let count_str: String = from(item_count.clone());
