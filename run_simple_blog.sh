...
echo "Compiling simple blog example..."
cargo run --bin nailc -- examples/blog_routes_simple.nail
...
if [ $? -eq 0 ]; then
    ...
    echo "Visit http://localhost:8082 to see the blog"
...
EOF
    ...
    cp ~/Nail/examples/blog_routes_simple.rs main.rs
...
EOF
    ...
    timeout 30 cargo run --manifest-path "$temp_dir/Cargo.toml" --bin blog_simple &
