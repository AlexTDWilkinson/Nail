...
mod example_tests {
    ...
    fn check_example(example: &str) -> Result<(), String> {
        let source = format!("nail latest\n{}\n", example);
        let tokens = crate::lexer::lexer(&source);
        let mut ast = crate::parser::parse(tokens).map_err(|error| format!("does not parse: {}", error.message.lines().next().unwrap_or("").trim()))?;
        crate::checker::checker(&mut ast).map_err(|errors| errors.first().map(|error| error.message.lines().next().unwrap_or("").trim().to_string()).unwrap_or_default())?;
        return Ok(());
    }

    #[test]
    fn every_stdlib_example_parses() {
        ...
        for (name, function) in STDLIB_FUNCTIONS.iter() {
            ...
            let source = format!("nail latest\n{}\n", function.example);
            if let Err(error) = crate::parser::parse(crate::lexer::lexer(&source)) {
                broken.push(format!("  {}: {}\n    {}", name, error.message.lines().next().unwrap_or("").trim(), function.example));
            }
        }
        broken.sort();
    ...
    fn every_stdlib_function_has_an_example() {
        ...
        missing.sort();
        assert!(missing.is_empty(), "these functions have no example to show: {:?}", missing);
    }
