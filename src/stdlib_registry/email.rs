...
pub(super) fn register(m: &mut HashMap<&'static str, StdlibFunction>) {
    m.insert("email_send_with_attachments", StdlibFunction {
        ...
        example: "server:EMAIL_Server = email_default_server();\ndanger(email_send_with_attachments(server, `billing@example.com`, `Your invoice`, `The invoice is attached.`, [EMAIL_Attachment { path = `invoice.pdf`, file_name = ``, mime_type = `` }]));",
    });
