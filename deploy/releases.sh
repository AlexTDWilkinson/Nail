...
$RELEASE_DOMAIN {
	# The launcher asks for a release by URL and writes it wherever it likes,
	# so the file on disk is named for the platform and nothing else. A browser
	# has no such choice: without this it saves a file called x86_64-linux,
	# with no version in the name and no extension, which is not something
	# anyone can carry to another machine and recognise later.
	@release path_regexp release ^/versions/([^/]+)/x86_64-linux\$
	header @release Content-Disposition "attachment; filename=nail-{re.release.1}-linux-x86_64.tar.xz"
