...
echo "-----------------------"
./test_lexer_parser.sh > "$TMP_DIR/lexer_parser.out" 2>/dev/null
grep -E "Summary:|Failed files:" -A 100 "$TMP_DIR/lexer_parser.out"
echo ""

echo "Stage 2: Type Checker"
echo "---------------------"
./test_type_checker.sh > "$TMP_DIR/type_checker.out" 2>/dev/null
grep -E "Summary:|Failed files:" -A 100 "$TMP_DIR/type_checker.out"
echo ""

echo "Stage 3: Transpiler"
echo "-------------------"
./test_transpiler.sh > "$TMP_DIR/transpiler.out" 2>/dev/null
