...
# Three files in examples/website/snippets/ had stopped compiling and one of
...
#                    pinned by ./scripts/test_error_messages.sh
...
nail_test_files() {
    ...
    while IFS= read -r file; do
        if head -3 "$PROJECT_ROOT/$file" | grep -q "not a standalone program"; then
            continue
        fi
        echo "$file"
