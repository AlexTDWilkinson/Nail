...
answer:i = danger(divide(10, 2));"#.to_string();
    ...
    let nav_links: Vec<String> = {
        ...
        for (_idx, item) in nav_items.clone().into_iter().enumerate() {
            __result.push(std_lib::array::join(vec! [r#"<a href=""#.to_string(), item.path.clone(), r#"" class="nav-link" hx-boost="true">"#.to_string(), item.name.clone(), "</a>".to_string()], "".to_string()).await);
        }
        __result
    };
...
"#.to_string();
    let website_html: String = std_lib::array::join(vec! [r#"
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
Hello, World!</pre>"#.to_string()).await;
    ...
    print_macro!(std_lib::array::join(vec! ["Starting Nail website on port ".to_string(), std_lib::string::from(port.clone()).await.unwrap_or_else(|nail_error| panic!("🔨 Nail Error: {}", nail_error)), "...".to_string()], "".to_string()).await);
    print_macro!("Visit http://localhost:8080 to see the Nail programming language website".to_string());
